...
import { buildApplyTaskFromSnapshot, ensureSnapshot } from '@/app/lib/moduleEngine';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload = await buildApplyTaskFromSnapshot(snapshot);
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to load apply task' },
      { status: 500 }
    );
  }
}
